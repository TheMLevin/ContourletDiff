...
ContourletResnetBlockBigGAN = layerspp.ContourletResnetBlockBigGANpp_Adagn
...
@utils.register_model(name='contourlet_ncsnpp')
class ContourletNCSNpp(NCSNpp):
    """NCSN++ model with Contourlet transform"""
    ...
    def __init__(self, config):
        ...
        self.no_use_freq = getattr(self.config, "no_use_freq", False)
        self.no_use_residual = getattr(self.config, "no_use_residual", False)
        self.nlevs = getattr(self.config, "contourlet_nlevs", 2)  # Default to 2 levels
        ...
        self.contour_dec = ContourDec(self.nlevs)
    ...
    def forward(self, x, time_cond, z):
        ...
        if self.no_use_fbn:
            ...
            h = modules[m_idx](h_lo / 2., temb, zemb)
            h = self.contour_rec([h * 2., h_hi])
        m_idx += 1
        ...
        if self.no_use_fbn:
            ...
            h = modules[m_idx](h, temb, zemb)
            h_lo, h_hi = self.contour_dec(h)
            h = modules[m_idx](h_lo / 2., temb, zemb)  # forward on contourlet space
            h = self.contour_rec([h * 2., h_hi])
        m_idx += 1
